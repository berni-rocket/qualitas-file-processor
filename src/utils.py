...
def generate_combinations(df, columns, columns_to_expand, delimiter='ý'):
    expanded_rows = []
    for idx, row in df.iterrows():
        fixed_values = [row[col] for col in columns]
        
        split_values = [row[col].split(delimiter) if delimiter in str(row[col]) else [row[col]] for col in columns_to_expand]
        
        for combination in itertools.product(*split_values):
            new_row = fixed_values + list(combination)
            expanded_rows.append(new_row)
    
    return pd.DataFrame(expanded_rows, columns=columns + columns_to_expand)