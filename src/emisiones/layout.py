...
class EmisionesLayoutMapper(LayoutMapper):
  ...
  def __init__(self, layout_file:str):
    ...
    def snake_case_transform(column):
      column = re.sub('\s{2,}', ' ', column).replace('.', '').replace(',',' ').replace('#','').replace('"','').replace('(','').replace(')','').replace('%','PC').replace('<','').replace('>','').replace('/',' ').replace(' ','_').lower().replace('__','_').replace('__','_')
    ...
    for record in table:
      ...
      if record['name'] == 'prima_total_me' and record['att'] == 99:
        name = 'prima_total_me_1'
      elif record['name'] == 'prima_total_me' and record['att'] == 107:
        name = 'prima_total_me_2'
