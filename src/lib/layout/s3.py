...
class S3LayoutMapper(LayoutMapper):
  
  def __init__(self, layout_file:str):
    self.layout_file = layout_file
    
    bucket_name = layout_file.replace('s3://','').split('/')[0]
    file_key = '/'.join(layout_file.replace('s3://','').split('/')[1:])
    
    s3 = boto3.client('s3')    
    file_object = s3.get_object(Bucket=bucket_name,Key=file_key) 
    file_object_content = file_object['Body'].read()
    file = io.BytesIO(file_object_content)
    file_string = file.getvalue().decode('utf-8')
    lineas = file_string.splitlines()
